    ...
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-hitm-navy/80 backdrop-blur-sm" onClick={() => setIsOpen(false)} />
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200 relative z-10">
            <div className="bg-hitm-navy p-5 flex justify-between items-center text-white border-b border-hitm-gold/30 relative overflow-hidden">
              ...
              <button
                ...
                className="text-white/70 hover:text-white transition-colors relative z-10 bg-white/10 rounded-full p-1.5 hover:bg-hitm-red"
              >
                <X size={18} />
              </button>
            </div>
            <div className="p-8 text-center text-hitm-navy">
              ...
              <div className="flex gap-4">
                <Button
                  variant="outline"
                  className="flex-1 h-12 uppercase tracking-wider text-xs font-bold border-gray-200 text-gray-500 hover:bg-gray-50"
                  onClick={() => setIsOpen(false)}
                >
                  Cancel
                </Button>
                <Button
                  className="flex-1 h-12 bg-hitm-red hover:bg-hitm-navy uppercase tracking-wider text-xs font-bold text-white shadow-lg shadow-hitm-red/20"
                  onClick={() => router.push('https://applynow.hitmranchi.ac.in/?utm_source=website')}
                >
                  Yes, Apply Now
                </Button>
              </div>
